...
import 'package:otp_field_bya2/otp_field_style_bya2.dart';
import 'package:otp_field_bya2/style_bya2.dart';
// Original credit goes to this package : otp_text_field
class OTPFieldByA2 extends StatefulWidget {
  ...
  @override
  _OTPFieldByA2State createState() => _OTPFieldByA2State();
}

class _OTPFieldByA2State extends State<OTPFieldByA2> {
  late OtpFieldStyle _otpFieldStyle;
  late List<FocusNode?> _focusNodes;
  late List<TextEditingController?> _textControllers;

  bool isFocusNodeSet = false;
  ...
  Widget buildTextField(BuildContext context, int index) {
    ...
    if (focusNode == null) {
      ...
      focusNode?.onKey = (node, RawKeyEvent event) {
        if (event is RawKeyDownEvent) {
          if (index != 0 && event.logicalKey == LogicalKeyboardKey.backspace) {
          ...
          final textFieldValue = _textControllers[index]?.text ?? "";
          if (textFieldValue.isNotEmpty && event.character != null) {
            final newValue = event.character ?? textFieldValue;
            if (int.tryParse(newValue) != null) {
              _textControllers[index]?.text = newValue;
              _onChanged(index, newValue, context);
              return KeyEventResult.handled;
            }
          }
        }
                    ...
                    onPressed: () {
                      Clipboard.getData(Clipboard.kTextPlain).then((value) {
                        final List<String> stringList =
                            value?.text?.split("") ?? [];
                        final List<String> toSetList = [];
                        for (int i = 0; i < widget.length; i++) {
                          toSetList.add(stringList.elementAtOrNull(i) ?? "");
                        }
                        if (_isInvalidIntegerList(toSetList)) {
                          log("Cannot Paste. Need Integer Value",
                              name: "OTP Field By A2");
                          return;
                        }
  ...
  bool _isInvalidIntegerList(List<String> stringList) {
    return stringList.fold(
      false,
      (previousValue, element) =>
      (int.tryParse(element) == null && element.isNotEmpty) || previousValue,
    );
  }

  void _onChanged(int index, String str, BuildContext context) {
...
class OtpFieldController {
  late _OTPFieldByA2State _otpTextFieldState;

  void setOtpTextFieldState(_OTPFieldByA2State state) {
